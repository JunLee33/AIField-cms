...
$(function() {
    ...
    if($("#current_user_grade").val() == '0101'){
        window.location.href = '/themamap/admin/cont?workspace_id='+now_workspaceID+'&workspace_nm='+workspace_nm;
    }
                ...
                {data : "create_date"},
                {
                    ...
                    render: function(data, type, full, meta){
                            if(data.themamap_status == "0201"){
                                ...
                                return "<strong value='"+data.themamap_status+"'>작업중 </strong><span id='pg_"+data.themamap_id+"'></span>";
    ...
    $("#btnInsertOpen").click(function() {
        window.location.href = '/themamap/cont/add?workspace_id='+now_workspaceID+'&workspace_nm='+workspace_nm
    });
...
function status_info(){
    ...
    themamap_id_List.filter((element, index) => {
        if(themamap_id_List.indexOf(element) === index){
            duplicate_check.push("J"+String(element));
        };
    });
    if(themamap_id_List.length == 0){
        return;
    }
...
function job_command(command, job_id, themamapNM){
    ...
    if(command == "detail"){
        ...
        window.location.href = '/themamap/cont/detail?themamap_id='+job_id+'&themamap_name='+themamapNM+'&workspace_id='+now_workspaceID;
        return;
    } else {
        var api_data = {
            "cmd" : command,
...
function set_progress(){
    ...
    for(var i in progress_list){
        var this_job_id = progress_list[i].job_id;
        this_job_id = this_job_id.replace("J", "");
        if(progress_list[i].status == 'working' ||  progress_list[i].status == 'suspend'){
            $("#pg_"+this_job_id).text("("+String(progress_list[i].progress)+")%")
        }else if(progress_list[i].status == 'done'){
            window.location.reload();
        }
    }
}